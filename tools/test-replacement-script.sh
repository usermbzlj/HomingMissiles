...
cp -- "$INSTALLER" "$UPLOAD/replace-homingmissiles-3.0.0.sh"
cp -- "$RELEASE_JAR" "$UPLOAD/HomingMissiles-3.0.0.jar"
...
HM_INSTALLER_TESTING=1 bash "$UPLOAD/replace-homingmissiles-3.0.0.sh"
...
HM_INSTALLER_TESTING=1 HM_INSTALLER_TEST_FAIL_AFTER_INSTALL=1 \
    bash "$UPLOAD/replace-homingmissiles-3.0.0.sh"
forced_rc=$?
set -e
