#!/usr/bin/env bash
# Isolated regression test for replace-homingmissiles-3.1.4.sh.

set -Eeuo pipefail
IFS=$'\n\t'

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
INSTALLER="$ROOT/tools/replace-homingmissiles-3.1.4.sh"
RELEASE_JAR="$ROOT/target/HomingMissiles-3.1.4.jar"
...
cp -- "$INSTALLER" "$UPLOAD/replace-homingmissiles-3.1.4.sh"
cp -- "$RELEASE_JAR" "$UPLOAD/HomingMissiles-3.1.4.jar"
...
HM_INSTALLER_TESTING=1 bash "$UPLOAD/replace-homingmissiles-3.1.4.sh"
installed_hash="$(hash_file "$SERVER/plugins/HomingMissiles-3.1.4.jar")"
...
cp -- "$SERVER/plugins/HomingMissiles-3.1.4.jar" "$SERVER/plugins/HomingMissiles-2.0.0.jar"
...
HM_INSTALLER_TESTING=1 HM_INSTALLER_TEST_FAIL_AFTER_INSTALL=1 \
    bash "$UPLOAD/replace-homingmissiles-3.1.4.sh"
forced_rc=$?
set -e
[[ "$forced_rc" -ne 0 ]] || { echo "forced failure unexpectedly succeeded" >&2; exit 1; }

[[ "$(hash_file "$SERVER/plugins/HomingMissiles-3.1.4.jar")" == "$installed_hash" ]]
