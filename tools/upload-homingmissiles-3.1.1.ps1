...
$ReleaseJarName = "HomingMissiles-3.1.1.jar"
$InstallerName = "replace-homingmissiles-3.1.1.sh"
...
$jarMatch = [regex]::Match(
    $checksumText,
    '(?im)^([0-9a-f]{64})[ \t]+HomingMissiles-3\.1\.1\.jar\s*$')
if (-not $jarMatch.Success) {
    Fail "$ChecksumName does not contain the 3.1.1 JAR checksum"
}
...
case "$bundle_real" in
    "$dest_real"/.homingmissiles-upload-3.1.1-*.zip) ;;
    *) echo "unsafe upload bundle path: $bundle_real" >&2; exit 1 ;;
esac

stage="$(mktemp -d "$dest_real/.homingmissiles-stage-3.1.1.XXXXXXXX")"
case "$stage" in
    "$dest_real"/.homingmissiles-stage-3.1.1.*) ;;
...
cleanup() {
    ...
    if [[ -n "${stage:-}" && -d "$stage" && "$stage" == "$dest_real"/.homingmissiles-stage-3.1.1.* ]]; then
        rm -rf -- "$stage"
    fi
    rm -f -- "$bundle_real"
}
...
cd "$stage"
required=(HomingMissiles-3.1.1.jar replace-homingmissiles-3.1.1.sh SHA256SUMS.txt deploy-config.properties)
if [[ "$include_hud" == 1 ]]; then
...
actual_jar="$(sha256sum -- HomingMissiles-3.1.1.jar | awk '{print tolower($1)}')"
[[ "$actual_jar" == "$expected_jar" ]] || { echo "remote JAR hash mismatch" >&2; exit 1; }
grep -Eiq "^${expected_jar}[[:space:]]+HomingMissiles-3\.1\.1\.jar[[:space:]]*$" SHA256SUMS.txt
grep -Fq "readonly EXPECTED_SHA256=\"$expected_jar\"" replace-homingmissiles-3.1.1.sh
bash -n replace-homingmissiles-3.1.1.sh
...
promote HomingMissiles-3.1.1.jar 0644
promote replace-homingmissiles-3.1.1.sh 0755
...
final_jar="$(sha256sum -- "$dest_real/HomingMissiles-3.1.1.jar" | awk '{print tolower($1)}')"
[[ "$final_jar" == "$expected_jar" ]] || { echo "promoted JAR hash mismatch" >&2; exit 1; }
printf 'UPLOAD=PASS\nREMOTE_DIR=%s\nJAR_SHA256=%s\n' "$dest_real" "$final_jar"
if [[ "$include_hud" == 1 ]]; then
    printf 'HUD_SHA1=%s\n' "$expected_hud"
fi
printf 'NEXT=sudo bash %s/replace-homingmissiles-3.1.1.sh\n' "$dest_real"
'@
