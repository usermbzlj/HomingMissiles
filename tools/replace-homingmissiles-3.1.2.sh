#!/usr/bin/env bash
# HomingMissiles 3.1.2 transactional replacement tool for Linux servers.
...
readonly RELEASE_VERSION="3.1.2"
readonly RELEASE_JAR_NAME="HomingMissiles-3.1.2.jar"
readonly EXPECTED_SHA256="f94ab1f3dc19f689f331dca4bafdcb71e8b618bde71eeaa37ae81fc8317c950f"
...
usage() {
    cat <<'USAGE'
HomingMissiles 3.1.2 一键替换工具

零参数用法：
  # 把 JAR、脚本和 deploy-config.properties 放在同一目录后执行：
  sudo bash replace-homingmissiles-3.1.2.sh

脚本默认读取同目录的 HomingMissiles-3.1.2.jar 与 deploy-config.properties。

脚本会按服务器 WorkingDirectory 自动识别唯一的 systemd 服务。也可以显式指定：
  bash replace-homingmissiles-3.1.2.sh \
    --jar /root/HomingMissiles-3.1.2.jar \
    --server-dir /opt/minecraft \
    --service minecraft.service

如果脚本与 HomingMissiles-3.1.2.jar 放在同一目录，可以省略 --jar。
...
参数：
  --jar PATH            待安装的 3.1.2 JAR；可省略并使用上述查找顺序
...
validate_release_jar() {
    ...
    grep -Eiq "^[[:space:]]*version:[[:space:]]*['\"]?3\.1\.2['\"]?[[:space:]]*(#.*)?$" <<<"$metadata" \
        || die "plugin.yml 的 version 不是 3.1.2"
...
new_log_has_start_marker() {
    ...
    tail -c "+$start_byte" -- "$latest" 2>/dev/null \
        | grep -aE 'Enabling HomingMissiles v3\.1\.2|HomingMissiles 3\.1\.2 .*已启用' >/dev/null
}
...
start_and_verify_new_release() {
    ...
    while (( SECONDS < deadline )); do
        ...
        if [[ "$state" == active ]] && (new_log_has_start_marker || journal_has_start_marker); then
            log INFO "服务处于 active，且已观察到 HomingMissiles 3.1.2 启用标记。"
            return 0
        fi
    ...
    die "在 ${STARTUP_TIMEOUT}s 内未同时确认服务 active 和插件 3.1.2 启用标记"
}
...
rollback() {
    ...
    if (( rollback_ok == 1 )); then
        log WARN "自动回滚已完成；失败的 3.1.2 文件保存在备份目录中供排查。"
    else
...
if (( ${#INSTALLED_JARS[@]} == 1 )) && [[ "${INSTALLED_JARS[0]}" == "$DEST_JAR" ]]; then
    ...
    if [[ "$installed_hash" == "$EXPECTED_SHA256" ]]; then
        log INFO "目标已是经过校验的 HomingMissiles 3.1.2；无需停服或重复替换。"
        COMMITTED=1
        exit 0
    fi
fi
...
sync -f "$PLUGIN_DIR"
log INFO "3.1.2 JAR 已原子安装：$DEST_JAR"
