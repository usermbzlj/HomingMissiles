...
```powershell
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File ".\tools\upload-homingmissiles-3.1.4.ps1"
```

也可以直接运行 `tools\upload-homingmissiles-3.1.4.cmd`。上传器读取两个不会提交到 Git 的本地配置。首次使用时复制模板：
...
```powershell
# 只检查并显示操作，不连接服务器
.\tools\upload-homingmissiles-3.1.4.ps1 -DryRun

# 临时覆盖配置中的主机或端口
.\tools\upload-homingmissiles-3.1.4.ps1 `
  -RemoteHostName staging.example.com -Port 2222

# 不上传 HUD 包
.\tools\upload-homingmissiles-3.1.4.ps1 -SkipHudPack
```
...
- `HomingMissiles-3.1.4.jar`
- `replace-homingmissiles-3.1.4.sh`
...
5. 在 `plugins/` 同一文件系统内原子安装 3.1.4；
6. 恢复服务，并在 `latest.log` 或 systemd journal 中验证 3.1.4 启用标记；
7. 任一步失败时移走新 JAR、恢复旧 JAR，并在服务原先运行时重新启动旧服务。

发布 JAR 的固定校验值：

```text
SHA-256  b7de313587f6ff704a86bb78f4a878c232c143c6da11823e109f779b37302233
```
...
`--install-only` 不会启动服务器；完成后需手动启动并检查日志。所有参数见 `bash replace-homingmissiles-3.1.4.sh --help`。
...
```bash
install -m 0644 HomingMissiles-3.1.4.jar /path/to/server/plugins/HomingMissiles-3.1.4.jar
```
...
```bash
mvn -U clean package
install -m 0644 target/HomingMissiles-3.1.4.jar /path/to/server/plugins/
```
...
```powershell
mvn -U clean package
Copy-Item .\target\HomingMissiles-3.1.4.jar C:\path\to\server\plugins\ -Force
```
...
```text
[HomingMissiles] Loading server plugin HomingMissiles v3.1.4
[HomingMissiles] Enabling HomingMissiles v3.1.4
[HomingMissiles] HomingMissiles 3.1.4 已启用
```
...
从配置版本 5 升至 6 时新增 `targeting.manual-lock.duration-ticks`、`cone-degrees`、`break-cone-degrees` 与 `break-grace-ticks`。6 版发射行为改为强制手动标定，`tracking.dynamic-retargeting` 和 `switch-advantage-blocks` 仅保留兼容读取。3.1.4 的均衡空战默认值为 18 tick、9°/14° 与 5 tick 容错；旧配置会继续保留自己的显式数值。

从配置版本 6 升至 7 时新增 `hud.client-mod.download-url` 与 `detection-grace-ticks`。旧配置缺少这两项时，插件会自动采用 v3.1.4 GitHub Release 页面与 40 tick 检测窗口。玩家个人 HUD 开关存于 PDC，不需要迁移 YAML。
