...
也可以直接运行 `tools\upload-homingmissiles-3.1.2.cmd`。上传器读取两个不会提交到 Git 的本地配置。首次使用时复制模板：
...
```powershell
# 只检查并显示操作，不连接服务器
.\tools\upload-homingmissiles-3.1.2.ps1 -DryRun

# 临时覆盖配置中的主机或端口
.\tools\upload-homingmissiles-3.1.2.ps1 `
  -RemoteHostName staging.example.com -Port 2222

# 不上传 HUD 包
.\tools\upload-homingmissiles-3.1.2.ps1 -SkipHudPack
```
...
```bash
bash replace-homingmissiles-3.1.2.sh \
  --config /root/deploy-config.properties \
  --jar /root/upload/HomingMissiles-3.1.2.jar \
...
5. 在 `plugins/` 同一文件系统内原子安装 3.1.2；
6. 恢复服务，并在 `latest.log` 或 systemd journal 中验证 3.1.2 启用标记；
7. 任一步失败时移走新 JAR、恢复旧 JAR，并在服务原先运行时重新启动旧服务。

发布 JAR 的固定校验值：

```text
SHA-256  f94ab1f3dc19f689f331dca4bafdcb71e8b618bde71eeaa37ae81fc8317c950f
```
...
`--install-only` 不会启动服务器；完成后需手动启动并检查日志。所有参数见 `bash replace-homingmissiles-3.1.2.sh --help`。
...
```bash
install -m 0644 HomingMissiles-3.1.2.jar /path/to/server/plugins/HomingMissiles-3.1.2.jar
```
...
```bash
mvn -U clean package
install -m 0644 target/HomingMissiles-3.1.2.jar /path/to/server/plugins/
```
...
```powershell
mvn -U clean package
Copy-Item .\target\HomingMissiles-3.1.2.jar C:\path\to\server\plugins\ -Force
```
...
```text
[HomingMissiles] Loading server plugin HomingMissiles v3.1.2
...
从配置版本 6 升至 7 时新增 `hud.client-mod.download-url` 与 `detection-grace-ticks`。旧配置缺少这两项时，插件会自动采用 v3.1.2 GitHub Release 页面与 40 tick 检测窗口。玩家个人 HUD 开关存于 PDC，不需要迁移 YAML。
