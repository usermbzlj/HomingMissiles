...
- `HomingMissiles-HUD-Fabric-1.21.11-3.1.2.jar`

把 Fabric API 和本 Mod JAR 放入客户端 `.minecraft/mods/`。进入安装了 HomingMissiles 3.1.2 插件的服务器后，Mod 会通过 `homingmissiles:control` 主动握手；服务端检测成功后不会再发送 HUD 资源包，并在聊天中确认“逐帧精确居中模式”。

`0.17.3` 是 Fabric API `0.141.6+1.21.11` 及其所有内嵌模块声明的真实 Loader 下限。v3.1.0 曾误将开发时使用的 `0.19.3` 写成运行时下限；v3.1.2 已去除这个不必要的限制。Minecraft `1.21.11` 和 Java 21 仍是必要条件。
