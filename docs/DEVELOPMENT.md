...
修改 Windows 上传器后，至少执行 `powershell.exe -NoProfile -File tools/upload-homingmissiles-3.1.1.ps1 -DryRun`，并用 Windows PowerShell 解析器检查语法。上传器、配置模板和 CMD 包装器保持 ASCII 内容，以兼容 Windows PowerShell 5 与中文工作区路径。不得提交 `tools/upload-config.properties` 或 `tools/deploy-config.properties`；只维护可公开的 `.example.properties`。
