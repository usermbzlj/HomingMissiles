...
- 修复 `/hbow tune` 查询表在读取 `lock-time` 时抛出“未映射的参数”并中断的问题，同时补齐 `lock-cone`。
- 每个在线可调参数现在在注册时必须同时提供当前值读取器，从结构上避免参数表与读取映射再次漏同步；离线测试会遍历全部 16 项。
- 将默认/`balanced` 配置调优为 2～4 人均衡空战：96/224 格捕获与保持、13 秒寿命、7°/tick 转向、3.2 巡航极速、4.8 后程极速，以及 18 tick / 9° 的主动锁定窗口。
